...
export function FoodSleepController(app: express.Express, db: mysql.Connection) {
    ...
    app.get("/food-sleep", (req: express.Request, res: express.Response) => {
        ...
        db.query('SELECT children.pk, children.name, children.group_id, food.date, food.breakfast, food.soup, food.main_dish, sleep.start_at, sleep.end_at' +
            ' FROM ((`children` LEFT JOIN food ON children.pk=food.child_id) LEFT JOIN sleep ON children.pk=sleep.child_id AND food.date=sleep.date)', [], (err: any, rows: any) => {
                if (err) {
                    res.json(err);
                    return;
                }
    ...
    app.get("/food-sleep/date/:date", (req: any, res: any) => {
        ...
        var query = `SELECT 
                ...
                s.start_at, s.end_at
            FROM \`children\` AS c 
                LEFT JOIN food AS f ON c.pk=f.child_id 
                LEFT JOIN sleep AS s ON c.pk=s.child_id AND f.date=s.date
            WHERE f.date=? OR f.date IS NULL`;
        var queryArgs = [req.params.date];
